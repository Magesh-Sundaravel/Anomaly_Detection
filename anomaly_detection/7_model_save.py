...
class MachineLearning:
    ...
    def preprocess_data(self):
        ...
        for file, data in data_dict.items():
            ...
            data['original_signal'] = scaler.fit_transform(data[column_name].values.reshape(-1, 1))

            columns_order = ['Day', 'Month', 'Year', 'Hour', 'Minute', 'original_signal',
...
class SupportVectorMachine(MachineLearning):
    ...
    def train_test_split(self):
        ...
        for file, (X, y, original_data) in processed_data.items():
            ...
            pipeline = Pipeline([('svc', SVC(C=1, kernel='linear', gamma='auto', random_state=101))])
            svc = pipeline.fit(X_train, y_train)

            # Save the model in the specified directory
            model_filename = os.path.join(self.model_save_dir, 'svm_model_1.pkl')
...
class RandomForest(MachineLearning):
    ...
    def train_test_split(self):
        ...
        for file, (X, y, original_data) in processed_data.items():
            X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2, random_state=101)
            clf = RandomForestClassifier(n_estimators=100, max_depth=10, random_state=101)
            rf = clf.fit(X_train, y_train)


            # Save the model in the specified directory
            model_filename = os.path.join(self.model_save_dir, 'rf_model_1.pkl')
